...
var hdr = []byte{0x67, 0x62, 0x6F, 0x6E, 0x00, 0x02}
...
var goldenCrossValue = []struct {
	...
	{"g52 repeat window", func(e *gbon.Encoder) {
		...
		_ = e.Encode(b)
		_ = e.Encode(b)
	}, "67626F6E0002 DC0D665B5D62797465 7101AA9002 C09002"},
	{"g53 sub window", func(e *gbon.Encoder) {
		v := []byte{01, 02, 03, 00}
		_ = e.Encode(v)
		_ = e.Encode(v[1:3])
	}, "67626F6E0002 DC0D665B5D62797465 74030102039002 C09202010203"},
	{"g54 tail mutation fallback", func(e *gbon.Encoder) {
		v := []byte{01, 02, 00, 00}
		_ = e.Encode(v)
		v[2] = 0xAA
		_ = e.Encode(v)
	}, "67626F6E0002 DC0D665B5D62797465 740201029002 C074030102AA9003"},
	{"g55 zerobase dedup", func(e *gbon.Encoder) {
		_ = e.Encode([]byte{})
		_ = e.Encode([]byte{})
	}, "67626F6E0002 DC0D665B5D62797465 70009002 C09002"},
}
...
func TestGoldenBigintMarshal(t *testing.T) {
	for _, tc := range goldenBigint {
		t.Run(tc.name, func(t *testing.T) {
			...
			if b[5] != 0x02 {
				t.Fatalf("header minor = %#x, want 02", b[5])
			}
			wb, err := hex.DecodeString(strings.ReplaceAll(tc.want, " ", ""))
			if err != nil {
				t.Fatalf("bad want hex: %v", err)
...
func TestGoldenV0LLegacyBigint(t *testing.T) {
	...
	want := append(append([]byte{0x67, 0x62, 0x6F, 0x6E, 0x00, 0x02},
